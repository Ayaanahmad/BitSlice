...
const Button = ({ children, disabled, to, type, soldOut, onClick }) => {
  ...
  const styles = {
    ...
    soldOut:
      "inline-block text-sm rounded-full bg-yellow-400 font-semibold tracking-wide text-stone-800 uppercase transition-colors duration-300 hover:bg-yellow-300 focus:bg-yellow-300 focus:ring focus:ring-yellow-300 focus:ring-offset-2 focus:outline-none disabled:cursor-not-allowed px-4 py-2 md:px-5 md:py-2.5 text-xs opacity-70 grayscale cursor-not-allowed",
  };
  if (to)
    return (
